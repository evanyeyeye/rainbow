...
class TestBinning(unittest.TestCase):
    """
    Unit tests for the shared (key, value) -> (time x ylabel) binning helper
    that replaced the per-scan np.unique / searchsorted / np.add.at loops in
    the Waters and Agilent spectrum decoders.
    ...
    def test_empty_input(self):
        ylabels, data = bin_datapairs(
            ...
            np.array([0, 0]), 0)
        self.assertEqual(ylabels.size, 0)
        self.assertEqual(data.shape, (2, 0))

    def test_matches_reference_on_random_data(self):
        rng = np.random.RandomState(0)
        for data_dtype in (np.int64, np.uint32):
            for prec in (0, 1, 2):
                for _ in range(20):
                    ...
                    self.assertEqual(d.dtype, d_ref.dtype)
